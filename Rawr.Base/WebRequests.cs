...
namespace Rawr
{
	public class WebRequestWrapper
	{
        ...
        public const string CONTENT_PNG = "image/png";
		...
		{
            ResetFatalErrorIndicator();//Added to test fix for creating directories.
			...
			_domains.Add(CharacterRegion.US, "wotlk");
        ...
        public XmlDocument DownloadItemSearch(string item)
        {
            ...
            if (item.Length > 0)
            {
                ...
                doc = DownloadXml(string.Format(NetworkSettingsProvider.ItemSearchURI, "wotlk", item));
            }
            return doc;
        }

        public XmlDocument DownloadItemWowhead(string id) { return DownloadItemWowhead("wotlk", id); }
        ...
        public XmlDocument DownloadItemHtmlWowhead(string id)
        {
            ...
            if (!string.IsNullOrEmpty(id))
            {
                ...
                doc = DownloadXml(string.Format("http://wotlk.evowow.com/?item={0}", id), true, true);
            }
            return doc;
        }
        ...
        public XmlDocument DownloadQuestHtmlWowhead(string id)
        {
            ...
            if (!string.IsNullOrEmpty(id))
            {
                doc = DownloadXml(string.Format("http://wotlk.evowow.com/?quest={0}", id), true, true);
            }
            return doc;
        }
		...
		public string DownloadTalentIcon(CharacterClass charClass, string talentTree, string talentName, string icon)
		{
            ...
            //try {
                ...
                if (icon != null) {
                    ...
                    uri = string.Format(NetworkSettingsProvider.ArmoryTalentIconURI, icon);// + ".jpg"; //Added .jpg to end
                    ...
                    if (!String.IsNullOrEmpty(talentTree) && !String.IsNullOrEmpty(talentName))
                    {
                        ...
                        //https://iradei.eu/talentcalcs/wotlk/{0}/images/{1}/{2}.jpg
		...
		{
            ServicePointManager.SecurityProtocol = (SecurityProtocolType)3072; //Added to prevent crash
			int retry = 0;
			bool success = false;
